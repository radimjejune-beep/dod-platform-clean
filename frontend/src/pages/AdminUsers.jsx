...
export default function AdminUsers() {
  ...
  const handleSubmitUser = async (e) => {
    ...
    try {
      ...
      // Очищаем телефон
      const cleanPhoneNumber = cleanPhone(form.phone);
  ...
  const handleImportUsers = async () => {
    ...
    for (const row of importPreview) {
      try {
        ...
        const password = generatePassword();
        const cleanPhoneNumber = cleanPhone(row.phone || '');
      ...
      <div className="container-page">
        <div className="page-header">
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <button className="btn-secondary" onClick={() => setShowImportModal(!showImportModal)}>
              📥 Импорт
            </button>
        ...
        {showImportModal && (
          <div className="card" style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '16px' }}>
              <button className="btn-secondary" onClick={downloadImportTemplate}>📄 Скачать шаблон</button>
                ...
                <tbody>
                  {(createdUsers.length > 0 ? createdUsers : importedUsersList).map((u, index) => (
                    <tr key={index} style={{ borderBottom: '1px solid #E2E7EF' }}>
                      ...
                      <td>{u.email}{u.is_auto_generated && <span style={{ marginLeft: '8px', fontSize: '10px', padding: '2px 6px', background: '#EAF2FA', color: '#174A7E', borderRadius: '4px' }}>Авто</span>}</td>
                      <td><code style={{ background: '#F4F6F9', padding: '2px 8px', borderRadius: '4px', fontSize: '13px', fontWeight: '600', color: '#0B1F3A' }}>{u.password}</code></td>
        ...
        {showCreateUser && canCreate && (
          <div className="card" style={{ padding: '24px', marginBottom: '24px' }}>
            ...
            <div style={{ padding: '8px 12px', marginBottom: '16px', background: '#EAF2FA', borderRadius: '8px', fontSize: '13px', color: '#174A7E' }}>
            ...
            <form onSubmit={handleSubmitUser}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
                <div className="form-group" style={{ border: '2px solid #C9A227', borderRadius: '10px', padding: '12px' }}>
                  <label style={{ fontWeight: '600', color: '#0B1F3A' }}>ФИО <span style={{ color: '#B3262E' }}>*</span></label>
                  <input type="text" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} required placeholder="Иванов Иван Иванович" style={{ border: '1.5px solid #C9A227' }} />
                </div>
                <div className="form-group" style={{ border: '2px solid #C9A227', borderRadius: '10px', padding: '12px' }}>
                  <label style={{ fontWeight: '600', color: '#0B1F3A' }}>Роль <span style={{ color: '#B3262E' }}>*</span></label>
                  <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} required style={{ border: '1.5px solid #C9A227' }}>
                ...
                {!editingUserId && (
                  <div className="form-group">
                    ...
                    <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="ivan@example.com" />
                    <div style={{ fontSize: '11px', color: '#98A2B3', marginTop: '4px' }}>Если не указан, будет сгенерирован автоматически</div>
                  </div>
                )}
                <div className="form-group">
                  <label>Телефон</label>
                  <input type="tel" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+7 999 123 45 67" />
                  <div style={{ fontSize: '11px', color: '#98A2B3', marginTop: '4px' }}>Введите номер без скобок, например: +7 999 123 45 67</div>
                </div>
                <div className="form-group">
                  <label>Школа</label>
                  <input type="text" value={form.school} onChange={(e) => setForm({ ...form, school: e.target.value })} placeholder="Школа №1" />
                </div>
                <div className="form-group">
                  <label>Класс</label>
                  <input type="text" value={form.class_name} onChange={(e) => setForm({ ...form, class_name: e.target.value })} placeholder="8А" />
                </div>
                <div className="form-group">
                  <label>Клуб</label>
                ...
                {editingUserId && (
                  <div className="form-group">
                    ...
                    <input type="email" value={form.email} disabled style={{ background: '#F4F6F9', cursor: 'not-allowed' }} />
                  </div>
                )}
              </div>
              {!editingUserId && (
                <div style={{ marginTop: '12px' }}>
                  <p style={{ fontSize: '13px', color: '#667085' }}>🔑 Пароль будет сгенерирован автоматически и показан после создания</p>
                </div>
              )}
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ flex: 1, minWidth: '200px' }}>
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="🔍 Поиск по ФИО, email, школе..." />
          </div>
          ...
          <div style={{ fontSize: '14px', color: '#667085' }}>Найдено: <strong>{users.length}</strong></div>
        </div>
        ...
        <div className="table-wrapper">
          <table>
            ...
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan="7" style={{ padding: '40px', textAlign: 'center', color: '#667085' }}>Пользователей не найдено</td></tr>
              ) : (
                users.map((u) => {
                  const userClub = clubs.find(c => c.id === u.club_id);
                  return (
                    <tr key={u.id}>
                      ...
                      <td>
                        {canChangeRoles ? (
                          <select value={u.role} onChange={(e) => handleRoleChange(u.id, e.target.value)} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid #D5DCE7', fontSize: '12px', background: 'white', cursor: 'pointer' }}>
                            <option value="participant">Участник</option>
                            <option value="parent">Родитель</option>
                            <option value="club_coordinator">Координатор КЮДа</option>
                            <option value="tutor">Тьютор</option>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '4px', justifyContent: 'center' }}>
                          ...
                          {canCreate && (
                            <button className="btn-secondary" style={{ padding: '4px 8px', fontSize: '12px', background: '#EAF2FA', color: '#174A7E' }} onClick={() => handleEditUser(u)}>✏️</button>
                          )}
                          {canDelete && u.id !== profile?.id && (
                            <button className="btn-danger" style={{ padding: '4px 8px', fontSize: '12px' }} onClick={() => handleDeleteUser(u.id, u.full_name)}>🗑️</button>
                          )}
                        </div>
                      </td>
                    </tr>
