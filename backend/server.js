...
function normalizeWord(value) {
  return String(value ?? '')
    ...
    .replace(/[’ʼ]/g, "'")
    .replace(/\s+/g, ' ')
    // Порядок важен: точку снимаем ПОСЛЕ обрезки пробелов. Иначе
    // «  Embassy. » — с пробелом в конце — не попадает под регулярное
    // выражение, точка остаётся, и верный ответ засчитывается как ошибка.
    // Ровно это и случилось на первой же живой проверке.
    .trim()
    .replace(/[.,!?;:]+$/, '')
    .trim();
}
