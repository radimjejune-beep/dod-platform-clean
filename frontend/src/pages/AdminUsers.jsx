...
export default function AdminUsers() {
  ...
  // Генератор логина жил здесь копией серверного и повторял его ошибки:
  // не выбрасывал скобки и русские буквы, поэтому «Проверкин Пётр Ильич
  // (можно удалить)» превращался в proverkin.udalit)123@dod.local, и
  ...
  const handleResetPassword = async (userId, fullName) => {
    ...
    try {
      ...
      if (!newPassword) {
        throw new Error('Сервер не вернул новый пароль — сообщите разработчику');
      }
  ...
  const handleSubmitUser = async (e) => {
    ...
    try {
      ...
      const newUserData = {
        ...
        password: result.temp_password || password,
        role: form.role,
        is_auto_generated: !email,
  ...
  const handleImportUsers = async () => {
    ...
    for (const row of importPreview) {
      try {
        // Пустой адрес выдаст сервер: он же разведёт совпадения.
        // Прежде на каждую строку файла заново скачивался весь список
        // пользователей — на сорока участниках это сорок запросов.
        const email = (row.email || '').trim();
        const isAutoGenerated = !email;
        let clubId = null;
        if (row.club) {
        ...
        {canCreate && (
          <div style={{
            ...
            {/* TODO: кнопка «Привязать родителя» скрыта намеренно. Форма в
                ...
                на сервере — нажатие приводило бы к ошибке. Вернуть, когда
                появится POST /api/parent-child для админа и координатора. */}
          </div>
        )}
