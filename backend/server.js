...
// ============================================================
// 10. СОЗДАНИЕ ПОЛЬЗОВАТЕЛЯ (С ГЕНЕРАЦИЕЙ EMAIL)
// ============================================================
app.post('/api/users', async (req, res) => {
  try {
    ...
    const existing = await pool.query('SELECT id FROM users WHERE email = $1', [email]);
    if (existing.rows.length > 0) {
      // Если email занят, добавляем суффикс
    ...
    res.status(201).json({
      ...
      email_generated: isAutoGenerated && !password // если email сгенерирован
    });
...
// ============================================================
// 16. СОБЫТИЯ (ЕДИНСТВЕННАЯ ВЕРСИЯ)
// ============================================================
...
// ============================================================
// ЗАПУСК СЕРВЕРА — ТОЛЬКО ОДИН РАЗ, В САМОМ КОНЦЕ!
// ============================================================
